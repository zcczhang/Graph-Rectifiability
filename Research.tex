...
\begin{problem}
    ...
    \end{equation}
\end{problem}
\textit{Proof}. For better notation, we write $Q_{k}$ as the dyadic cube with side length $2^{-k}, k\in\mathbb{N}$ in the fixed cube system. Then for any two different points in $Q_k$, we have $0<|x_1-x_2|\leq \sqrt{C}\cdot 2^{-k}, \forall x_1, x_2\in Q_k$ where $C$ is the constant and particularly $C=n$ when in $\RR^n$. When $k\rightarrow \infty, 2^{-k}\rightarrow 0$ so there exists $N\in\NN$ such that whenever $k\geq N$, we have $|x_1-x_2|<\alpha\epsilon/2<\epsilon/2, \forall \epsilon >0$ for choosing $x_1, x_2$ still in the $Q_k\ni x$. Now consider that if $y\in C_\mathcal{B}(x_1, V, \alpha)$, we have $\operatorname{dist}(y-x_1, V) > \alpha|y-x_1|$ by definition of bad cones. Choose
$$\epsilon\leq \frac{\operatorname{dist}(y-x_1, V)-\alpha|y-x_1|}{\alpha}\Rightarrow \operatorname{dist}(y-x_1, V) \geq \alpha (|y-x_1|+\epsilon),$$
so that
\begin{equation*}
    \begin{aligned} 
        \operatorname{dist}\left(y-x_{2}, V\right) & \geq \operatorname{dist}(y-x_1, V)-\left|x_1-x_2\right| \\ 
